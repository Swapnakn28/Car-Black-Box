...
void putch(unsigned char byte) 
{
	...
	while(!TXIF)    //TXIF =0- full,1-empty
	{
		continue;
	}
	TXIF = 0;
...
//transmitting a string TXREG is 8 bit,so transimitting byte by byte
...
//receing and transmitting 1 byte
unsigned char getche(void)
{
	unsigned char c;

	putch(c = getch()); //reading 1 byte and transmitting same 1 byte
